...
כללים מחייבים:
- דיוק מתמטי, לוגי ולשוני מוחלט. בדוק בעצמך כל שלב פתרון לפני שאתה כותב שאלה — חייבת להיות בדיוק תשובה נכונה אחת מתוך 4 האפשרויות, ללא אפשרות לפרשנות אחרת שתוביל לתשובה שונה.
...
- שאלות כמותיות (section = quant): נסח כל גודל ומשתנה במפורש לפני השימוש בו, ציין תמיד יחידות מידה (₪, ק"מ, שעות וכו') כשרלוונטי, והרכב את השאלה כך שקיימת דרך פתרון חד-משמעית אחת בלבד — לא ניסוח שמאפשר שתי אסטרטגיות פתרון לגיטימיות המובילות לתוצאות מספריות שונות (למשל עיגול לא-מוגדר, "בערך" ללא טווח סטייה מוגדר, או יחס שיכול להתפרש בשני כיוונים).
- שאלות מילולי מכל סוג (השלמת משפטים, הבנה והסקה לוגית, הבנת הנקרא, אנלוגיות): גזרת השאלה (ה-stem) חייבת להיות תמציתית וישירה — משפט אחד או שניים, ללא פסקת מבוא מיותרת ולא פיתולי תחביר שמאלצים קריאה חוזרת כדי להבין מה נדרש. כל נתון הדרוש לפתרון מופיע במפורש בגוף השאלה או בקטע הנלווה — לעולם לא בהנחה סמויה שהנבחן "יבין לבד".
- אסור ניסוח מסורבל, מלאכותי, מורכב מדי, או שנשמע כמו תרגום מכונה — בגוף השאלה, באפשרויות ובהסבר כאחד. כל משפט חייב להישמע כמו שפה טבעית שכתב דובר שפת-אם (עברית בקטעים כמותי ומילולי, אנגלית בקטע אנגלית), לא כמו טקסט מתורגם מילה-במילה. אם משפט דורש קריאה שנייה כדי להבין את כוונתו, נסח אותו מחדש בפשטות לפני שאתה כותב אותו לשדה הסופי.
