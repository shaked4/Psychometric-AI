...
export async function GET() {
  try {
    ...
    try {
      ...
      console.error("[sync/pull] auth() failed, treating as clerk_not_configured:", error);
      return NextResponse.json({ pulled: false, reason: "clerk_not_configured" });
    }

    if (!userId) {
      // Guest / signed-out caller — nothing to pull yet, not a server
      // error, so this stays a clean 200 rather than a 401.
      return NextResponse.json({ pulled: false, reason: "unauthenticated" });
    }
    ...
    if (attemptsRes.error || examHistoryRes.error || questionsRes.error) {
      console.error("[sync/pull] Supabase query failed:", {
        attemptsError: attemptsRes.error,
        examHistoryError: examHistoryRes.error,
    ...
    // Catch-all for anything not already handled above — previously this
    // would surface in Vercel as a bare 500 with no way to tell what
    // actually happened.
    console.error("[sync/pull] Unhandled error:", error);
    return NextResponse.json({ pulled: false, reason: "unexpected_error" }, { status: 500 });
  }
}
